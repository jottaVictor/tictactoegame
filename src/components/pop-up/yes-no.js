...
export function YesNoPopUp({title = 'Alerta!', description='', timeToClose=5000}){    
    const { 
        expandGenericPopUp,
        ...
        <div className={`generic-pop-up ${isActive ? 'active' : ''} ${isExpanded ? 'expanded' : ''}`}>
            ...
            <div className='pop-up-footer'>
                <div className='box-option'>
                    <label className='no-botton' title='Recusar' onClick={() => noFunc()}>Não</label><span className="material-symbols-outlined">close</span>
                </div>
                <div className='box-option'>
                    <label className='yes-botton' title='Confirmar' onClick={() => yesFunc()}>Sim</label><span className="material-symbols-outlined">check</span>
                </div>
            </div>
        </div>
    )
